...
export async function parseSequence(content, fileName) {
    const seqs = parseFile(content, { fileName });
    if (seqs.length === 0)
        throw new Error(`no sequence parsed from ${fileName}`);
    return seqs[0];
}
/**
 ...
 */
export async function parseSequenceFromBuffer(buffer, fileName) {
 ...
 *  defined. */
export function toSeqvizAnnotations(annotations) {
