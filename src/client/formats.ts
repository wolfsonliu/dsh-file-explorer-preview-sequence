import { SEQUENCE_FORMATS } from '../protocol.ts'
...
export function formatLabelFor(ext: string): string | null {
  ...
  return SEQUENCE_FORMATS[key as keyof typeof SEQUENCE_FORMATS]
}
...
export function fallsBackToText(ext: string): boolean {
  const key = ext.toLowerCase()
